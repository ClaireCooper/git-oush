...
@test "git oush plays sound" {
    run git oush
    assert_output -p '.wav 0'
    assert_success
}

@test "git oush -pp plays very quiet sound" {
    run git oush -pp
    assert_output -p '.wav '"$pp_volume"
    assert_success
}

@test "git oush -p plays quiet sound" {
    run git oush -p
    assert_output -p '.wav '"$p_volume"
    assert_success
}

@test "git oush -mp plays slightly quiet sound" {
    run git oush -mp
    assert_output -p '.wav '"$mp_volume"
    assert_success
}

@test "git oush -mf plays slightly loud sound" {
    run git oush -mf
    assert_output -p '.wav '"$mf_volume"
    assert_success
}

@test "git oush -f plays loud sound" {
    run git oush -f
    assert_output -p '.wav '"$f_volume"
    assert_success
}
...
@test "git oush -ff plays really loud sound" {
    run git oush -ff
    assert_output -p '.wav '"$ff_volume"
    assert_success
}
